...
def GetImageBgHist(img, channel, paras_rb = [50, 50]):
    ...
    bg_normal = img_as_uint(rank.mean())
...
def CalcFluoByRegion(img, mask, centroid0, ids0, filename):
    yfp_bg_reduced, yfp_bg, yfp_hist = GetImageBgHist(img, 'YFP')
    mc_bg_reduced, mc_bg, mc_hist = GetImageBgHist(img, 'mcherry')
    ...
    c0, c1 = 'YFP', 'mcherry'
    ax[0, 0].imshow(img[..., CHANNEL[c0]], cmap = 'gray')
    ...
    ax[1, 0].imshow(img[..., CHANNEL[c1]], cmap = 'gray')
    ...
    ax[2, 1].imshow(yfp_bg_reduced * mask, cmap = 'gray')
    ax[2, 1].set_title("%s masked" % c0)
    ax[2, 1].axis('off')
    ax[2, 2].imshow(mc_bg_reduced * mask, cmap = 'gray')
    ax[2, 2].set_title("%s masked" % c1)    
    ax[2, 2].axis('off')
    ...
    df = pd.DataFrame.from_dict(stats, orient='index').reset_index().rename(columns={'index': 'Region'})
    ...
    for parent, folder, file in walk(mask_dir):
        for f in file:
            ...
            if pos_mask == position:
                ...
                centroid0, ids0, stats = CalcFluoByRegion(img, mask, centroid0, ids0, f)
                dict_df[tp_mask] = stats
